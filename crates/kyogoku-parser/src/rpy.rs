...
impl Parser for RpyParser {
    ...
    fn parse(&self, content: &[u8]) -> Result<Vec<TranslationBlock>> {
        ...
        while let Some((line_idx, line)) = lines.next() {
            ...
            if (trimmed.starts_with("python") || trimmed.starts_with("init python")) && trimmed.ends_with(':') {
            ...
            if let Some((speaker, quote_str, is_closed)) = parse_multiline_start(line) {
                if is_closed {
                    ...
                    blocks.push(TranslationBlock::new(content.to_string())
    ...
    fn serialize(&self, blocks: &[TranslationBlock], template: &[u8]) -> Result<Vec<u8>> {
        ...
        for block in blocks {
            let type_ = block.metadata.get("type").and_then(|v| v.as_str()).unwrap_or("dialogue");
            ...
            if type_ == "multiline_dialogue" || type_ == "multiline_dialogue_oneline" {
                ...
                let start = start_opt
                    ...
                    .ok_or_else(|| anyhow::anyhow!("Missing line metadata for block {}", block.id))? as usize;
...
fn parse_multiline_start(line: &str) -> Option<(Option<String>, String, bool)> {
    ...
    let speaker = if prefix.is_empty() { None } else { Some(prefix.to_string()) };
