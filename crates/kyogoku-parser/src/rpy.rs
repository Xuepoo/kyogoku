...
impl Parser for RpyParser {
    ...
    fn serialize(&self, blocks: &[TranslationBlock], template: &str) -> Result<String> {
        ...
        for block in blocks {
            let line_idx = block.metadata.get("line").and_then(|v| v.as_u64()).map(|v| v as usize);
            let quote = block.metadata.get("quote").and_then(|v| v.as_str()).unwrap_or("\"");
            let output_text = block.output();
            
            if let Some(idx) = line_idx {
                if let Some(original_line) = lines.get_mut(idx) {
...
fn parse_dialogue_line(line: &str) -> Option<(Option<String>, String, String)> {
    ...
    if trimmed.starts_with("scene ") || trimmed.starts_with("show ") || 
       trimmed.starts_with("play ") || trimmed.starts_with("stop ") ||
       trimmed.starts_with("define ") || trimmed.starts_with("default ") ||
       trimmed.starts_with("label ") || trimmed.starts_with("jump ") ||
       trimmed.starts_with("return") || trimmed.starts_with("call ") ||
        ...
        if let Some(space_idx) = trimmed.find(' ') {
            ...
            if let Some(first_char) = rest.chars().next() {
                if first_char == '"' || first_char == '\'' {
                    if let Some(content) = extract_string_content(rest, first_char) {
...
fn parse_menu_choice(line: &str) -> Option<(String, String)> {
    ...
    if first_char == '"' || first_char == '\'' {
        if let Some(content) = extract_string_content(content_part, first_char) {
