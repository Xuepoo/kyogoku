...
impl Parser for JsonParser {
    ...
    fn serialize(&self, blocks: &[TranslationBlock], template: &str) -> Result<String> {
        let json: Value = serde_json::from_str(template)
        ...
        match value {
            ...
            Value::Object(nested) => {
                ...
                if let Some(Value::String(source)) = nested.get("original").or(nested.get("source")) {
                    ...
                    if let Some(Value::String(target)) = nested.get("translation").or(nested.get("target"))
                        && !target.is_empty() {
                    ...
                    Some("mtool") => {
                        if let Value::Object(nested) = value {
                            if nested.contains_key("translation") {
                                nested.insert("translation".to_string(), Value::String(block.output().to_string()));
                            } else if nested.contains_key("target") {
                                nested.insert("target".to_string(), Value::String(block.output().to_string()));
                            }
                        }
                    }
                    _ => {}
...
fn write_array(mut arr: Vec<Value>, blocks: &[TranslationBlock]) -> Value {
    for block in blocks {
        let idx = block.metadata.get("index").and_then(|v| v.as_u64()).map(|v| v as usize);
        let format = block.metadata.get("format").and_then(|v| v.as_str());
                    ...
                    Some("dialogue") => {
                        if let Value::Object(obj) = value {
                            if obj.contains_key("text") {
                                obj.insert("text".to_string(), Value::String(block.output().to_string()));
                            } else if obj.contains_key("message") {
                                obj.insert("message".to_string(), Value::String(block.output().to_string()));
                            }
                        }
                    }
                    _ => {}
