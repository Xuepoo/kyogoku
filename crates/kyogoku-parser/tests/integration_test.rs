...
"#.as_bytes();
    ...
    let registry = ParserRegistry::new();
    let parser = registry
        .get_parser(std::path::Path::new("test.vtt"))
        .unwrap();
...
fn test_parser_selection() {
    ...
    assert!(
        registry
            .get_parser(std::path::Path::new("test.unknown"))
            .is_none()
    );
}
...
fn test_rpy_basic() {
    ...
    assert!(
        blocks
            .iter()
            .any(|b| b.source == "You've created a new Ren'Py game.")
    );
    assert!(
        blocks
            .iter()
            .any(|b| b.source == "This is a narration line.")
    );
    assert!(blocks.iter().any(|b| b.source == "It's a story."));

    // Check speaker
    let eileen_line = blocks
        .iter()
        .find(|b| b.source.contains("Ren'Py game"))
        .unwrap();
    ...
    if let Some(b) = translated_blocks
        .iter_mut()
        .find(|b| b.source == "It's a story.")
    {
